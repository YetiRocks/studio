...
export function SpansPanel({ paused, onTogglePause }: SpansPanelProps) {
  ...
  const filtered = items.filter(span => {
    if (appFilter && span.appId !== appFilter) return false
    if (search) {
      const detail = spanDetail(span)
      if (!span.name.toLowerCase().includes(searchLower)
        && !span.target.toLowerCase().includes(searchLower)
        && !detail.toLowerCase().includes(searchLower)) return false
    }
    return true
  })
  ...
  return (
    <div className="panel">
      <div className="panel-toolbar">
        <select className="app-filter" value={appFilter} onChange={e => setAppFilter(e.target.value)}>
          <option value="">All apps</option>
          {appIds.sort().map(id => <option key={id} value={id}>{id}</option>)}
        </select>
        <input
        ...
        {filtered.length === 0 ? (
          ...
          filtered.map((span, i) => (
            <div key={span.id || i} className="span-entry">
              ...
              {span.appId && <span className="span-app">{span.appId}</span>}
              <span className="span-detail">{spanDetail(span)}</span>
...
function spanDetail(span: SpanEntry): string {
  if (!span.fields) return span.name
  try {
    const f = JSON.parse(span.fields)
